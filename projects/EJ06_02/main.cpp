...
glm::mat4 lookAt(const glm::vec3 position, const glm::vec3 objective, const glm::vec3 up)
{
    glm::vec3 cameraDirection = glm::vec3(glm::normalize(position - objective));
    glm::vec3 cameraRight = glm::vec3(glm::normalize(glm::cross(up, cameraDirection)));
    ...
    Result[3][0] = -glm::dot(cameraRight, position);
    Result[3][1] = -glm::dot(cameraUp, position);
    Result[3][2] = -glm::dot(cameraDirection, position);
    return Result;
...
void render(const Geometry& geom, const Shader& shader, Texture& tex) {
    ...
    shader.set("view", lookAt(camera.getPosition(),camera.getFront(),camera.getUp()));
