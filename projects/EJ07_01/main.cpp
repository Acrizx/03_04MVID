...
void render(const Geometry& object, const Geometry& light, const Shader& s_phong, const Shader& s_light) {
    ...
    float grados = static_cast<float>(glfwGetTime()*5);
    /*double calculos = glm::cos(grados) ;
    if ((grados >= 30 && grados <= 35) || (grados >= 145 && grados <= 150)) {
        float a = grados;
    }*/
    lightPos = glm::vec3(glm::cos(grados) * 4.0f, 0.0f, glm::sin(grados) * 4.0f);
