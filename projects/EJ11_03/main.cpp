...
void render(const Geometry& floor, const Geometry& object, const Shader& s_phong,
    const Texture& t_albedo, const Texture& t_specular, const Shader& s_light) {
    ...
    s_phong.set("light.ambient", 0.2f, 0.1f, 0.1f);
    ...
    s_phong.set("color", glm::vec4(1.0f, 1.0f, 1.0f, 1.0f));

    floor.render();

    for (int i = 0; i < sizeof(objectPos) / sizeof(objectPos[0]); i++)
    {
        model = glm::mat4(1.0f);
        ...
        s_phong.set("color", objectColor[i]);
        object.render();
    }
}
...
int main(int, char* []) {
    ...
    const Shader s_light("../projects/EJ11_03/light.vs", "../projects/EJ11_03/light.fs");
    ...
    while (window->alive()) {
        ...
        render(quad, quad, s_phong, t_albedo, t_specular, s_light);
        window->frame();
    }
