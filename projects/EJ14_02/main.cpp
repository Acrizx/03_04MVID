...
//glm::vec3 lightPos2(3.0f, 4.0f, -3.0f);
//glm::vec3 lightDir2(0.0f, 0.0f, 0.0f);
...
void render(const Geometry& quad, const Geometry& cube, const Geometry& sphere, const Geometry& light,
    ...
    for (uint32_t i = 0; i < (sizeof(spotLightPositions) / sizeof(spotLightPositions[0])); ++i) {
        const glm::mat4 lightProjection = glm::perspective<float>(glm::radians(100.0f), 1.0, k_shadow_near, k_shadow_far);
        const glm::mat4 lightView = glm::lookAt(spotLightPositions[i], glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        lightSpaceMatrix[i] = lightProjection * lightView;
        //glCullFace(GL_BACK);
        s_depth.use();
        s_depth.set("lightSpaceMatrix", lightSpaceMatrix[i]);
        //glCullFace(GL_FRONT);
        renderScene(s_depth, quad, cube, sphere, t_albedo, t_specular);

    }
    //s_depth.use();
    //s_depth.set("lightSpaceMatrix", lightSpaceMatrix[0]+ lightSpaceMatrix[1]);
    ////glCullFace(GL_FRONT);
    //renderScene(s_depth, quad, cube, sphere, t_albedo, t_specular);
    //glCullFace(GL_BACK);
    ...
    //s_depth.use();
    //s_depth.set("lightSpaceMatrix", lightSpaceMatrix2 + lightSpaceMatrix);
    ////glCullFace(GL_FRONT);
    //renderScene(s_depth, quad, cube, sphere, t_albedo, t_specular);
...
//SECOND PASS
    ...
    for (uint32_t i = 0; i < (sizeof(spotLightPositions) / sizeof(spotLightPositions[0])); ++i) {
        ...
        s_phong.set((lightName + "cutOff").c_str(), glm::cos(glm::radians(glm::radians(40.0f))));
        s_phong.set((lightName + "outerCutOff").c_str(), glm::cos(glm::radians(glm::radians(50.0f))));
    }

    s_phong.set("material.shininess", 32);

    s_phong.set("lightSpaceMatrix", lightSpaceMatrix[0]);
    s_phong.set("lightSpaceMatrix2", lightSpaceMatrix[1]);
...
int main(int, char* []) {
    ...
    while (window->alive()) {
        ...
        render(quad, cube, sphere, light, s_phong, s_depth, s_debug, s_light, t_albedo, t_specular, fbo.first, fbo.second);
        window->frame();
    }
